    ...
    return [
        {
            ...
            "score": float(p.score),
            "rank_source": "vector",
            "objection_text": p.payload.get("objection_text", ""),
            "response_text": p.payload.get("response_text", ""),
        }
        for p in response.points
        if p.payload and p.payload.get("case_id")
    ]
...
async def _fts_search(
    ...
    if category:
        cat_filter = "AND category = :category::objection_category"
    else:
        cat_filter = ""
    if customer_segment:
        seg_filter = "AND customer_segment = :segment::customer_segment"
    else:
        seg_filter = ""
    if car_brand:
        brand_filter = "AND car_brand = :car_brand"
    else:
        brand_filter = ""
    if fluid_type:
        fluid_filter = "AND fluid_type = :fluid_type"
    else:
        fluid_filter = ""
    ...
    params = {"q": query, "tenant_id": tenant_id, "limit": FTS_TOP_K}
    if category:
